...
pages=()
for c in `curl $URL$CLASS/'page'[14-20]/`
do 
...
do
    ...
    #mp3=$URL`curl -s $URL$p | grep -oP "download\/[0-9]{6}\/"`
    mp3=`curl -s $URL$p | grep -oP "http:\/\/t1.g.hjfile.cn\/listen\/[0-9]{6}\/[0-9]{20,22}.mp3"`
    ...
    echo URL: $mp3
    title=沪江听写酷_`echo $title | tr ' ' '_' | tr '/' '-' `
